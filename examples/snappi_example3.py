...
def hello_snappi():
    ...
    eth1, ip1, udp1, udp12 = f1.packet.ethernet().ipv4().udp().udp()
    eth2, ip2, udp2, udp22 = f2.packet.ethernet().ipv4().udp().udp()
    ...
    ip1.version.values = [4, 15]

    ip1.priority.raw.values = [255, 0, 45, 218, 117]

    ip1.identification.increment.start = 50000
    ip1.identification.increment.step = 100
    ip1.identification.increment.count = 500
    ip1.reserved.value = 1
    ip1.dont_fragment.increment.start = 0
    ip1.dont_fragment.increment.step = 1
    ip1.dont_fragment.increment.count = 1
    ip1.time_to_live.increment.start = 50
    ip1.time_to_live.increment.step = 1
    ip1.time_to_live.increment.count = 255
    ...
    req.port_name = 'p2'
    # with open('tests/data/pcap/ipv4/ipv41_p2.pcap', 'wb') as f:
